...
fn devcontainer_build_simple() {
    ...
    let executor = ExecutorResources::setup(&home, "build-test");
...
fn devcontainer_build_with_args() {
    ...
    let executor = ExecutorResources::setup(&home, "build-args");
...
fn devcontainer_build_with_target() {
    ...
    let executor = ExecutorResources::setup(&home, "build-target");
...
fn devcontainer_build_reuses_cached_image() {
    ...
    write_devcontainer_with_build(&repo, "Dockerfile");
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "build-cache");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn devcontainer_build_with_context() {
    ...
    write_devcontainer_with_build(&repo, "Dockerfile");
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "build-context");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn devcontainer_build_skips_when_image_exists() {
    ...
    write_devcontainer_with_build(&repo, "Dockerfile");
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "build-skip");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn no_devcontainer_enters_with_default_image() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "default-image");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn devcontainer_build_streams_output() {
    ...
    write_devcontainer_with_build(&repo, "Dockerfile");
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "build-stream");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
