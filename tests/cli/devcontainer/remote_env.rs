...
fn remote_env_simple() {
    ...
    let executor = ExecutorResources::setup(&home, "renv-simple");
...
fn remote_env_local_env_substitution() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "renv-local");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn remote_env_container_env_substitution() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "renv-cenv");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn remote_env_available_in_agent() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "renv-agent");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn remote_env_not_in_container_processes() {
    ...
    let executor = ExecutorResources::setup(&home, "renv-bg");
...
fn user_env_probe_login_shell() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "probe-login");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
...
fn user_env_probe_none() {
    ...
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home, "probe-none");
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.toml"), &executor.toml).unwrap();
