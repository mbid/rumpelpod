...
/// first session spawn.
fn setup_install_test_repo(test_name: &str) -> (TestHome, TestRepo, ExecutorResources, TestDaemon) {
...
fn claude_auto_install() {
    let (home, repo, _executor, daemon) = setup_install_test_repo("claude-auto-install");
