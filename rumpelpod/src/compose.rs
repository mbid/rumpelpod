...
mod tests {
    ...
    fn model() -> Model {
        Model::parse(
            r#"{"services":{"agent":{"image":"base"},"db":{"image":"database","ports":[{"target":5432,"published":"5432"}]}}}"#
                .to_string(),
        )
        .unwrap()
    }
    ...
    #[test]
    fn override_labels_only_the_agent_as_a_pod() {
        let yaml = generate_override(
            &model(),
            "agent",
            "dev",
            "prepared",
        ...
        assert_eq!(yaml.matches("container_name: !reset null").count(), 2);
        assert_eq!(
            yaml.matches("\"dev.rumpelpod.name\"").count(),
            1,
            "only the agent service may carry the pod-name label"
        );
        let pod_label = yaml.find("\"dev.rumpelpod.name\"").unwrap();
        let image = yaml.find("    image: \"prepared\"").unwrap();
        assert!(
            pod_label < image,
            "agent label entries must remain nested under labels"
        );
    }

    #[test]
    fn override_declares_devcontainer_named_volumes() {
        let mounts = [MountObject {
            mount_type: MountType::Volume,
            source: Some("shared-cache".to_string()),
        ...
        assert!(yaml.contains("source: \"rumpelpod_mount_0\""));
        assert!(yaml.contains("target: \"/cache\"\n        read_only: true"));
        assert!(yaml.contains("\"rumpelpod_mount_0\":\n    name: \"shared-cache\""));
    }

    #[test]
    fn built_image_can_be_persisted_without_its_source_context() {
        let mut model = model();
        model
            .set_built_service_image("agent", "sha256:abc".to_string())
            .unwrap();
        assert_eq!(model.service_image("agent").unwrap(), "sha256:abc");
    ...
    #[test]
    fn selected_services_always_include_agent() {
        let devcontainer = DevContainer {
            run_services: Some(vec!["db".to_string()]),
            ..DevContainer::default()
        };
        assert_eq!(
            selected_services(&devcontainer, "agent"),
            vec!["db".to_string(), "agent".to_string()]
        );
    }
}
