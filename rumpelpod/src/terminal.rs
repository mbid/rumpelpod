...
async fn terminal_ws_handler(
    ...
    ws.on_upgrade(move |socket| async move {
        let result = match kind.as_str() {
            "codex" => serve_codex(socket, daemon, &query, &name).await,
...
async fn serve_codex(
    ...
    let codex_cli_path = query
        .codex_cli_path
        .as_deref()
        .context("codex terminal requires codex_cli_path")?;
    let spec =
        crate::codex::build_codex_spec(&daemon, &query.repo_path, pod, codex_cli_path).await?;
    crate::pty_session::serve_ws_session_with_params(socket, daemon.pty_sessions(), spec).await;
    Ok(())
}
