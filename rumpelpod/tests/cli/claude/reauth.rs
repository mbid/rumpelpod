...
//! credentials from the local machine into the container without needing
//! to recreate the pod.
...
fn reauth_updates_oauth_credentials() {
    ...
    // Write new OAuth credentials on the local machine.
    ...
    // Run reauth.
    rumpel(&repo, &daemon, &home)
        .args(["claude", "--create", "test", "reauth"])
        .success()
        .expect("reauth");
...
fn reauth_updates_api_key() {
    ...
    // Put a new API key on the local machine.
    std::fs::write(
        home.path().join(".claude.json"),
        r#"{"primaryApiKey":"sk-new-key-12345","hasCompletedOnboarding":true}"#,
    )
    .expect("write local .claude.json with new key");
