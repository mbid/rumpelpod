...
//!
//! This test creates a real user on the local machine, installs the
//! rumpelpod daemon as a systemd user service for that user, exercises
//! enter/delete, then uninstalls and cleans up.  Because it mutates the
//! local machine (useradd, userdel, loginctl enable-linger), it only
//! runs when RUMPELPOD_TEST_SYSTEMD=1.
    ...
    if std::env::var(ENV_VAR).as_deref() != Ok("1") {
        panic!(
            "Refusing to run: {ENV_VAR} is not set to 1.\n\
             This test creates/deletes users and systemd services on the local machine.\n\
             Set {ENV_VAR}=1 to opt in (e.g. inside the devcontainer)."
        );
    }
}

/// A local-machine user that is deleted on drop.
struct TestUser {
    name: String,
    home: String,
}
...
impl TestUser {
    fn create() -> Self {
        ...
        // enable-linger on its own does not actually bring up the
        // per-user manager in recent systemd; it only arms the unit
        // so that logins (and explicit starts) keep it alive after
        // logout.  Start user@UID.service ourselves so the user bus
        // at /run/user/<uid>/bus exists before `rumpel system-install`
        // tries to `systemctl --user daemon-reload`.
        let user_uid = uid_of(&name);
        Command::new("sudo")
            .args(["systemctl", "start", &format!("user@{user_uid}.service")])
            .success()
            .unwrap_or_else(|e| panic!("starting user@{user_uid}.service failed: {e}"));

        // Wait for the user bus to actually come up.  `is-system-running`
        // reaches the user manager over dbus; exit 0 means "running"
        // and exit 1 means "degraded" -- either is fine, we only care
        // that the bus is reachable.
        ...
        loop {
            let ok = Command::new("sudo")
                .args([
                    ...
                    &name,
                    "env",
                    &format!("XDG_RUNTIME_DIR=/run/user/{user_uid}"),
                    "systemctl",
                    "--user",
                    "is-system-running",
                    "--wait",
    ...
    fn cmd(&self, program: &str) -> Command {
        ...
        cmd.args([
            ...
            &self.name,
            "env",
            &format!("PATH={}", std::env::var("PATH").expect("PATH not set")),
...
fn systemd_install_enter_delete_uninstall() {
    ...
    // Write a devcontainer.json + .rumpelpod.json for a minimal pod.
    ...
    write_as_user(&user, &format!("{repo_dir}/.rumpelpod.json"), "{}\n");
    ...
    let enter_out = user
        .shell_in(
            &repo_dir,
            "rumpel enter --create systemd-test -- echo hello-from-pod",
        )
        .output()
