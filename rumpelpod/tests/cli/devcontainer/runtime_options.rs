...
fn write_devcontainer_with_runtime_opts(repo: &TestRepo, runtime_opts: &str) {
    ...
    let dockerfile = formatdoc! {r#"
        ...
        RUN apk add --no-cache git shadow procps
...
fn privileged_mode_enabled() {
    ...
    pod_command(&repo, &daemon)
        .args(["enter", "--create", "privileged-test", "--", "true"])
        .success()
        .expect("rumpel enter failed");
}
...
fn cap_add_sys_ptrace() {
    ...
    pod_command(&repo, &daemon)
        .args(["enter", "--create", "cap-add-test", "--", "true"])
        .success()
        .expect("rumpel enter failed");
}
...
fn security_opt_seccomp_unconfined() {
    ...
    pod_command(&repo, &daemon)
        .args(["enter", "--create", "seccomp-test", "--", "true"])
        .success()
        .expect("rumpel enter failed");
}
...
fn host_requirements_logged() {
    ...
    let output = pod_command(&repo, &daemon)
        .env("RUST_LOG", "info")
        .args(["enter", "--create", "hostreq-logged", "--", "true"])
        .output()
...
fn host_requirements_ignored_on_local() {
    ...
    pod_command(&repo, &daemon)
        .args(["enter", "--create", "hostreq-absurd", "--", "true"])
        .success()
