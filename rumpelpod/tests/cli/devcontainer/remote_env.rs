...
fn remote_env_local_env_substitution() {
    ...
    let stdout = pod_command(&repo, &daemon)
        ...
        .args(["enter", "--create", "renv-local", "--", "printenv", "VAR"])
        .success()
        .expect("rumpel enter failed");
...
fn remote_env_container_env_substitution() {
    ...
    let stdout = pod_command(&repo, &daemon)
        .args(["enter", "--create", "renv-cenv", "--", "printenv", "PATH"])
        .success()
        .expect("rumpel enter failed");
...
/// and confirming the variable is present each time.
#[test]
fn remote_env_available_on_repeated_exec() {
    let repo = TestRepo::new();

    write_devcontainer_with_remote_env(&repo, r#"{ "REPEAT_TEST_VAR": "repeat_value" }"#);
    let home = TestHome::new();
    let executor = ExecutorResources::setup(&home);
    let daemon = TestDaemon::start(&home);
    fs::write(repo.path().join(".rumpelpod.json"), &executor.json).unwrap();
    ...
    let stdout = pod_command(&repo, &daemon)
        ...
        .expect("rumpel enter failed");
    let stdout = String::from_utf8_lossy(&stdout);
    assert_eq!(stdout.trim(), "repeat_value");

    // Second exec into the same pod. remoteEnv must be injected every time,
    // not just once.
    let stdout = pod_command(&repo, &daemon)
        .args([
            "enter",
...
/// started via `rumpel enter`. A background process started by the
...
fn user_env_probe_login_shell() {
    ...
    let stdout = pod_command(&repo, &daemon)
        .args(["enter", "--create", "probe-login", "--", "printenv", "PATH"])
        .success()
        .expect("rumpel enter failed");
...
fn user_env_probe_none() {
    ...
    let stdout = pod_command(&repo, &daemon)
        .args(["enter", "--create", "probe-none", "--", "printenv", "PATH"])
        .success()
        .expect("rumpel enter failed");
